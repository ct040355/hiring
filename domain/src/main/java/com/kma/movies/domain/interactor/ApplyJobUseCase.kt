...
class ApplyJobUseCase(private val moviesRemoteRepository: MoviesRemoteRepository) {
    fun execute(username: String, id: String): Single<RegisterStatus> {
        return moviesRemoteRepository.applyCV(username, id)
    }
